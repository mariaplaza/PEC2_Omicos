...
```{r echo=FALSE, message=FALSE, warning=FALSE, tidy=TRUE}
et <- exactTest(disp.estim, pair = c("NIT", "ELI"))
...
```{r warning=FALSE,message=FALSE,tidy=TRUE,fig.height=4.5}
...
El número total de genes con un valor de FDR inferior al nivel de significación (α = 0,05) fue de 17422. De estos genes, 3590 se identificaron como infrarregulados y 2099 como sobrerregulados.
...
plotSmear(et, de.tags=detags, main="plotSmear entre los grupos NIT y ELI")
...
hist(top.1$table$FDR, breaks=100, main="Histograma de FDR (SFI vs NIT)")
abline(v=0.05, col="red", lwd=3)
...
plotSmear(et.1, de.tags=detags.1, main="plotSmear entre los grupos SFI y NIT")
abline(h=c(-1,1), col="blue")
...
hist(top.2$table$FDR, breaks=100, main="Histograma de FDR (SFI vs ELI)")
abline(v=0.05, col="red", lwd=3)
...
plotSmear(et.2, de.tags=detags.2, main="plotSmear entre los grupos SFI y ELI")
abline(h=c(-1,1), col="blue")
...
Podemos representar esta función con el siguiente grafico, que indica la media de los conteos normalizados de cada gen frente a su dispersión por gen y a la dispersión común o curva ajustada a
...
```{r echo=FALSE, fig.height=6.5, message=FALSE, warning=FALSE, tidy=TRUE}
...
```{r warning=FALSE,message=FALSE,tidy=TRUE,fig.height=7}
...
La salida de esta función proporcionó el número de genes que presentaban un valor de logFC significativamente negativo (3262 genes) o significativamente positivo (2556 genes) a un nivel de significación de 0,05, con solo un valor outlier.
...
```{r echo=FALSE, fig.height=5, message=FALSE, warning=FALSE, tidy=TRUE}
#reset par
par(mfrow=c(1,1))
...
res1 <- results(dds.1, contrast=c("condition","ELI","SFI"), alpha = 0.05)
summary(res1)
...
Los resultados de la comparativa entre los datos de RNA-seq de la condición “ELI” y “SFI”, con un p-valor ajustado de 0.05 (padj), mostraron un total de 17675 genes diferencialmente expresados, frente a los 46109 genes del estudio (res1). Es estos 17675, 3258 estaban sobreexpresados (un 38% de los genes y un 7.1% para los genes totales), con solo un valor outliers, 5835 genes con un p-ajustado menor a 0.05.
...
Como resultado, se han obtenido un total de 6247 genes upregulados y 2072 downregulados en la primera comparación (ELI y SFI).
...
res2 <- results(dds.1, contrast=c("condition","ELI","NIT"), alpha = 0.05)
summary(res2)
...
Los resultados de la comparativa entre los datos de RNA-seq de la condición “ELI” y “NIT”, con un p-valor ajustado de 0.05 (padj), mostraron un total de 15908 genes diferencialmente expresados, frente a los 46109 genes del estudio (res2). Es estos 15908, 3201 estaban sobreexpresados (un 35% de los genes y un 6.9% de los genes totales), con un valor outlier.
...
Como resultado, se han obtenido un total de 6239 genes upregulados y 2182 downregulados en la segunda comparación (ELI y NIT)
...
res3 <- results(dds.1, contrast=c("condition","NIT","SFI"), alpha = 0.05)
summary(res3)
...
Los resultados de la comparativa entre los datos de RNA-seq de la condición “NIT” y “SFI”, con un p-valor ajustado de 0.05 (padj), mostraron un total de 20326 genes diferencialmente expresados, frente a los 46109 genes del estudio (res3). Es estos 20326, 29 estaban sobreexpresados (un 44% para los diferencialmente expresados, un 0.063% de los genes totales), con un valor outlier.
...
Como resultado, se han obtenido un total de 2337 genes upregulados y 2820 downregulados en la tercera comparación (NIT y SFI)
...
```{r warning=FALSE,message=FALSE,tidy=TRUE,fig.height=6.5}

DESeq2::plotMA(res1, main="MA-plot DESeq2", ylim=c(-2,2))
abline(h=c(-1,1), col="dodgerblue", lwd=2)
DESeq2::plotMA(res2, main="MA-plot DESeq2", ylim=c(-2,2))
abline(h=c(-1,1), col="dodgerblue", lwd=2)
DESeq2::plotMA(res3, main="MA-plot DESeq2", ylim=c(-2,2))
abline(h=c(-1,1), col="dodgerblue", lwd=2)
...
# El nombre de las filas pertenece a las claves de tipo "ENSEMBL", conteniendo un punto y la versión de éste. Eliminamos la version y el punto para que posteriormente podamos emplear la AnnotationDBI para mapear mediante DESeq, si es necesrio
...
res.ord1 <- res1[order(res1$padj),]
head(res.ord1)[,1:6] # tabla de resultados ordenados por p-value

resadj1 = subset(res1, padj < 0.05)
summary(resadj1) # padj menores a 0.05
...
```{r eval=FALSE, fig.height=6.5, message=FALSE, warning=FALSE, include=TRUE, tidy=TRUE}
